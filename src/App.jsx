...
export default function App() {
  ...
  const [path, setPath] = useState([]); // [ {type: 'year', title: 'السنة 1'}, {type: 'semester', title: 'السمستر 1'} ]
  ...
  // واجهة تسجيل الدخول مع البلور والخلفية المطلوبة
          ...
          <div className="max-w-4xl mx-auto animate-in slide-in-from-top-4 duration-500">
            <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 mb-8">
              ...
              <form onSubmit={addMaterial} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-gray-400 uppercase mr-2">اسم المادة</label>
                  <input name="subject" required placeholder="مثلاً: جيوديسيا" className="w-full p-4 bg-gray-50 rounded-2xl border-none font-bold focus:ring-2 focus:ring-[#f58220]" />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-gray-400 uppercase mr-2">عنوان الدرس</label>
                  <input name="title" required placeholder="مثلاً: مقدمة المساحة" className="w-full p-4 bg-gray-50 rounded-2xl border-none font-bold focus:ring-2 focus:ring-[#f58220]" />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-gray-400 uppercase mr-2">الرابط المباشر</label>
                  <input name="url" required placeholder="YouTube / Drive Link" className="w-full p-4 bg-gray-50 rounded-2xl border-none font-bold focus:ring-2 focus:ring-[#f58220] text-left" />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-gray-400 uppercase mr-2">السنة الدراسية</label>
                  <select name="yearId" className="w-full p-4 bg-gray-50 rounded-2xl border-none font-bold">
                    {years.map(y => <option key={y.id} value={y.id}>{y.title}</option>)}
                  </select>
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-gray-400 uppercase mr-2">السمستر</label>
                  <select name="semesterId" className="w-full p-4 bg-gray-50 rounded-2xl border-none font-bold">
                    {semesters.map(s => <option key={s.id} value={s.id}>{s.title}</option>)}
                  </select>
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-gray-400 uppercase mr-2">نوع المحتوى</label>
                  <select name="type" className="w-full p-4 bg-gray-50 rounded-2xl border-none font-bold">
                    <option value="video">محاضرة فيديو</option>
                    <option value="pdf">ملف ملخص / شيت</option>
                    <option value="exam">نموذج امتحان سابق</option>
                  </select>
                </div>
            ...
            <div className="bg-white rounded-[2.5rem] border border-gray-100 overflow-hidden shadow-sm">
                <div className="p-6 border-b border-gray-50 flex justify-between items-center">
                  <h3 className="font-black text-gray-700">المواد الحالية بالمنصة ({contents.length})</h3>
                  <div className="text-xs text-[#f58220] font-bold">نظام الربط التلقائي فعّال ✅</div>
                </div>
                <div className="divide-y divide-gray-50">
                   {contents.map(item => (
                     <div key={item.id} className="p-4 sm:p-6 flex items-center justify-between hover:bg-gray-50 transition-colors">
                       <div className="flex items-center gap-4">
                          <div className={`p-3 rounded-xl ${item.type === 'video' ? 'bg-red-50 text-red-500' : 'bg-indigo-50 text-indigo-500'}`}>
                            {item.type === 'video' ? <Video size={20}/> : <FileText size={20}/>}
                          </div>
                          <div>
                            <p className="font-bold text-gray-800">{item.subject} <span className="text-gray-300 font-normal px-1">|</span> {item.title}</p>
                            <p className="text-[10px] text-gray-400 font-bold uppercase mt-1">سنة {item.yearId} • سمستر {item.semesterId} • {item.type}</p>
                          </div>
                       </div>
                   ...
                   {contents.length === 0 && <p className="p-10 text-center text-gray-400">لا يوجد محتوى دراسي مضاف حالياً.</p>}
                </div>
            </div>
          </div>
        ) : (
          /* --- واجهة الطالب (نظام التنقل الهرمي) --- */
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {years.map(y => (
                  <button key={y.id} onClick={() => setPath([{type: 'year', ...y}])} className="bg-white p-12 rounded-[2.5rem] border border-gray-100 shadow-sm hover:shadow-2xl hover:border-[#f58220]/20 hover:-translate-y-2 transition-all group text-right relative overflow-hidden">
                    ...
                    <div className="bg-[#f58220]/10 w-16 h-16 rounded-2xl flex items-center justify-center text-[#f58220] mb-8 group-hover:bg-[#f58220] group-hover:text-white transition-all duration-500 shadow-inner">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 max-w-4xl mx-auto">
                {semesters.map(s => (
                  <button key={s.id} onClick={() => setPath([...path, {type: 'semester', ...s}])} className="bg-white p-16 rounded-[3rem] border-2 border-transparent hover:border-[#f58220] shadow-xl text-center group transition-all">
                    <div className="w-24 h-24 bg-[#f58220]/5 rounded-full flex items-center justify-center mx-auto mb-8 text-[#f58220] group-hover:rotate-12 transition-all">
                       <BookOpen size={48} />
                    </div>
                    <h3 className="text-3xl font-black text-gray-800">{s.title}</h3>
                    <p className="text-[#f58220] font-bold text-xs mt-4 uppercase">المواد الأكاديمية</p>
                  </button>
                ))}
              </div>
            )}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                {Array.from(new Set(contents.filter(c => c.yearId === path[0].id && c.semesterId === path[1].id).map(c => c.subject))).map(sub => (
                  <button key={sub} onClick={() => setPath([...path, {type: 'subject', title: sub}])} className="bg-white p-8 rounded-[2rem] border border-gray-100 hover:shadow-xl hover:bg-[#f58220]/5 transition-all text-center group">
                    ...
                    <span className="text-[10px] text-gray-300 font-bold mt-2 block uppercase tracking-widest">عرض الدروس</span>
                  </button>
                ))}
                {contents.filter(c => c.yearId === path[0].id && c.semesterId === path[1].id).length === 0 && (
                  <div className="col-span-full py-20 text-center opacity-30">
                    <p className="text-2xl font-black">لا توجد مواد مرفوعة لهذا السمستر</p>
                  </div>
                )}
              </div>
            )}

            {/* 4. عرض الدروس داخل المادة أو تشغيل المحتوى */}
                ...
                {contents.filter(c => c.yearId === path[0].id && c.semesterId === path[1].id && c.subject === path[2].title).map(item => (
                  <button key={item.id} onClick={() => setSelectedItem(item)} className="bg-white p-6 rounded-[2.5rem] border border-gray-50 shadow-sm flex items-center justify-between hover:shadow-xl hover:-translate-y-1 transition-all group text-right">
                    <div className="flex items-center gap-5">
                      <div className={`w-16 h-16 rounded-[1.25rem] flex items-center justify-center ${item.type === 'video' ? 'bg-red-50 text-red-500' : 'bg-indigo-50 text-indigo-500'}`}>
                        {item.type === 'video' ? <Video size={30} /> : <FileText size={30} />}
                      </div>
                      <div>
                        <h4 className="font-black text-xl text-gray-800">{item.title}</h4>
                        <p className="text-[10px] text-gray-400 font-black uppercase mt-1 italic tracking-wider">{item.type === 'video' ? 'محاضرة فيديو' : 'ملخص PDF'}</p>
                      </div>
                    </div>
                    <div className="p-3 bg-gray-50 rounded-full group-hover:bg-[#f58220] group-hover:text-white transition-all shadow-inner">
                       <ChevronLeft size={20} />
                    </div>
                  </button>
                ))}
              ...
              <div className="bg-white rounded-[3rem] shadow-2xl overflow-hidden border border-gray-100 animate-in slide-in-from-bottom-8 duration-500">
                <div className="p-4 bg-gray-50/50 flex items-center justify-between border-b border-gray-100">
                   <div className="flex items-center gap-2">
                      <div className="w-8 h-8 rounded-full border border-gray-200 p-1 bg-white">
                         <img src={GESS_LOGO} className="w-full h-full object-contain" />
                      </div>
                      <span className="text-xs font-black text-gray-500">{selectedItem.subject}</span>
                   </div>
                ...
                {selectedItem.type === 'video' ? (
                  <div className="aspect-video bg-black relative">
                    <iframe src={selectedItem.url.replace("watch?v=", "embed/")} className="w-full h-full border-none" allowFullScreen></iframe>
                  </div>
                ) : (
                  <div className="py-24 px-10 text-center bg-gradient-to-br from-[#f58220]/5 to-transparent">
                    ...
                    <p className="text-gray-400 mb-10 font-bold italic">هذا الملف متاح للتحميل والقراءة المباشرة</p>
                    <a href={selectedItem.url} target="_blank" rel="noreferrer" className="bg-[#f58220] text-white px-12 py-5 rounded-[2rem] font-black shadow-2xl shadow-[#f58220]/30 hover:scale-105 transition-transform inline-flex items-center gap-3">
                       <Plus size={24}/> فتح الملف الآن
                    </a>
                  </div>
                )}
                ...
                <div className="p-10 border-t border-gray-50">
                  <div className="flex items-center gap-3 mb-4">
                     <span className="bg-[#f58220] text-white text-[10px] font-black px-4 py-1.5 rounded-full uppercase tracking-tighter">GESS EDU</span>
                     <span className="text-gray-200">|</span>
                     <span className="text-gray-400 text-xs font-bold italic underline">{selectedItem.subject}</span>
                  </div>
                  <h2 className="text-4xl font-black text-gray-900 leading-tight">{selectedItem.title}</h2>
                  <p className="mt-4 text-gray-500 font-medium leading-relaxed max-w-2xl">تم رفع هذا المحتوى بواسطة قسم هندسة الجيوماتيك - جامعة الخرطوم لخدمة الطلاب أكاديمياً وتسهيل الوصول للمصادر التعليمية.</p>
                </div>
              </div>
            )}
          </div>
